...
class LocalRapidMinerKernel private[kernel] (
...
/*class LocalRapidMinerKernelFactory extends Kernel.Factory.LocalService {
  import LocalRapidMinerKernel.kernelCounter
...
//    session          <- startSparkSession(sparkJars, classLoader)
    ...
    scalaInterpreter <- interpreters.getOrCreate("scala")(ScalaRapidMinerInterpreter().provideSomeM(Env.enrich[Blocking](compiler)))
    interpState      <- Ref[Task].of[State](State.predef(State.Root, State.Root))
  } yield new LocalRapidMinerKernel(compiler, session, interpState, interpreters, busyState)
...
object LocalRapidMinerKernel extends LocalRapidMinerKernelFactory {
